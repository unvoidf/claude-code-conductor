...
- **NEVER** output only an announcement (e.g., "I will now begin...") without also performing an action in the same turn.
...
1.  **Check for User Input:** First, check if the user provided a track name as an argument via `$ARGUMENTS`.

2.  **Locate and Parse Tracks Registry:**
    -   Resolve the **Tracks Registry**.
    -   Read and parse this file. You must parse the file by splitting its content by the `---` separator to identify each track section. For each section, extract the status (`[ ]`, `[~]`, `[x]`), the track description (from the `**Track:` line), and the link to the track folder.
...
1.  **Update Status to 'In Progress' and Announce:**
    -   Before beginning any work, you MUST update the status of the selected track in the **Tracks Registry** file.
    -   This requires finding the specific line for the track (e.g., `- [ ] **Track: <Description>**`) and replacing `[ ]` with `[~]` in the **Tracks Registry** file you identified earlier.
    -   While performing this update, briefly inform the user which track you are beginning to implement.
...
2.  **Load Track Specification (and briefly inform the user you are synchronizing docs):** Read the track's **Specification**.
