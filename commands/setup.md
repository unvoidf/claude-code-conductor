...
- **NEVER** output only an announcement (e.g., "I will now create...") without also performing an action or asking a question in the same turn.
- When transitioning between sections, **always combine** the transition announcement with either a file operation or the next question.
- If a step says "Announce X, then proceed to Y", you MUST do both in the **same turn**: announce X and immediately perform Y's first action or ask Y's first question.
- The ONLY time you should end a turn with just text is when you are asking the user a question and need their response.

**PLUGIN TEMPLATE RESOLUTION:** When this protocol requires copying template files (such as workflow.md or code styleguides), you must locate the conductor plugin's `templates/` directory. To find it:
1. Search for a directory containing `.claude-plugin/plugin.json` with `"name": "conductor"`.
2. Common locations: the plugin directory provided via `--plugin-dir`, or `~/.claude/plugins-cache/`.
3. The template directory is at `<plugin_root>/templates/`.
4. If templates cannot be found, generate the content inline based on the default workflow template and styleguide principles.
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            ...
            -   **2.1 File Size and Relevance Triage:**
                ...
                2.  **Efficiently List Relevant Files:** To list the files for analysis, you MUST use a command that respects the ignore files. For example, you can use `git ls-files --exclude-standard -co | head -100` which lists relevant files (tracked by Git, plus other non-ignored files). If Git is not used, list files manually while excluding common build/dependency directories.
                3.  **Fallback to Manual Ignores:** ONLY if `.gitignore` does not exist, you should fall back to manually ignoring common directories like `node_modules`, `.m2`, `build`, `dist`, `bin`, `target`, `.git`, `.idea`, `.vscode`.
...
2.  **Select Code Style Guides:**
    -   **Locate templates:** Find the conductor plugin's `templates/code_styleguides/` directory using the **Plugin Template Resolution** protocol defined at the top of this document. List the available style guide files.
    ...
    -   **Action:** Create the directory `conductor/code_styleguides/` and copy the selected style guide files from the plugin's template directory into it.
    -   **Commit State:** Upon successful completion of the copy, you MUST immediately write to `conductor/setup_state.json` with the exact content:
        `{"last_successful_step": "2.4_code_styleguides"}`

### 2.5 Select Workflow (Interactive)
1.  **Copy Initial Workflow:**
    -   Locate the conductor plugin's `templates/workflow.md` file using the **Plugin Template Resolution** protocol. Copy it to `conductor/workflow.md`.
    -   If the template file cannot be found, generate a default workflow.md with TDD methodology, >80% coverage requirement, per-task commits, and git notes for task summaries.
2.  **Customize Workflow:**
    -   Ask the user: "Do you want to use the default workflow or customize it?"
        ...
        -   A) Default
        -   B) Customize
    -   **If the user chooses Default (Option A):**
        -   Keep the workflow file as-is. No modifications needed.
...
3.  **Commit State (REQUIRED for both Default and Customize paths):** After the `workflow.md` file is successfully written or confirmed, you MUST immediately write to `conductor/setup_state.json` with the exact content:
    `{"last_successful_step": "2.5_workflow"}`
4.  **CRITICAL: Do NOT stop here. Do NOT wait for user input. Immediately proceed to Section 2.6.**

### 2.6 Finalization
**CRITICAL: This entire section must be executed as a single continuous action. Do NOT output text and wait for user input. Perform ALL file operations first, then present the summary alongside the first question of Section 3.0.**
...
2.  **CRITICAL: Do NOT stop here. Immediately continue to Section 3.0 in the same turn.** When you present the Phase 1 summary, combine it with the first question of Section 3.0 so the user has something to respond to.
...
**CRITICAL: Do NOT make announcements without an accompanying action or question. Every turn you take MUST either perform a tool action (file read/write) or ask the user a question. Never output only an announcement and stop.**

### 3.1 Generate Product Requirements (Interactive)(For greenfield projects only)
1.  **Transition to Requirements:** In the same message where you summarize Phase 1 completion, immediately read `conductor/product.md` and present the first requirement question. Do NOT make a separate announcement turn.
...
1.  **Generate and Present Track Title:** Analyze the project context (`product.md`, `tech-stack.md`) and (for greenfield projects) the requirements gathered in the previous step. Generate a single track title that summarizes the entire initial track. Present it to the user together with a brief explanation that a "track" is a high-level unit of work (like a feature or bug fix) used to organize the project. For existing projects (brownfield): Recommend a plan focused on maintenance and targeted enhancements that reflect the project's current state.
...
**CRITICAL: Once the track is approved, immediately begin creating ALL artifacts below in a single continuous action. Do NOT announce what you will do and stop — just do it.**

1.  **Initialize Tracks File:** Create the `conductor/tracks.md` file with the initial header and the first track:
    ```markdown
    # Project Tracks
