...
2.  **Check for a User-Provided Target:** First, check if the user provided a specific target via `$ARGUMENTS`.
...
3.  **Identify the Track Creation Commit (Track Revert Only):**
    ...
    *   **Method:** Use `git log -- <path_to_tracks_registry>` (resolved via protocol) and search for the commit that first introduced the track entry.
        *   Look for lines matching either `- [ ] **Track: <Track Description>**` (standard format) OR `## [ ] Track: <Track Description>` (heading format).
